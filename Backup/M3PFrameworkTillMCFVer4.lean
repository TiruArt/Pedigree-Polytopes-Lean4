  ...
  if targetNode.k ≠ 6 then false
    ...
    | some lastNode =>
      if lastNode.k ≠ 5 then false
        ...
        -- Rule [1]: (a,b) ≠ (iₗ,jₗ) for ℓ ∈ {4,5}
        let notInPath := P.nodes.all fun node =>
          if node.k >= 4 then
            not (node.i = a && node.j = b)
          else
            true

        -- Rule [2]: If b > 3, then a ∈ {iᵦ, jᵦ}
...
def computeLinkCapacity (R₄ : List (PedigreePath 4))
  ...
  let extendable := R₄.filter fun P =>
    canPedigreeExtendTo6 P tgtNode
...
def buildF5Network (X : ParsedMIRData n)
  ...
  let pedigreeArcs := R₄.foldl (init := []) fun acc P =>
    let validTargets := layer6.filter fun tgt =>
      canPedigreeExtendTo6 P tgt
...
def computeF5MaxFlow (net : F5Network) : Rat :=
  ...
  let sinkFlows := net.sinkNodes.map fun (sinkNode, sinkCap) =>
    ...
    let totalIncoming := fromArcs + fromPedigrees
    min totalIncoming sinkCap

  let totalSinkCapacity := sinkFlows.foldl (fun acc cap => acc + cap) 0

  min totalSupply totalSinkCapacity
...
def checkM3PMembership (X : ParsedMIRData n) (hn : n ≥ 5) : IO (MembershipResult n) := do
  ...
  -- STEP 2: Analyze F₅
  if n >= 6 then
    let (f5Result, f5Status) ← analyzeF5Complete X f4Result.R₄ f4Result.μ hn
    match f5Status with
    | StageResult.failure msg =>
