...
-- ============================================================================
-- STEP 2 - F_k ANALYSIS FOR k ≥ 5
-- ============================================================================

/-- STEP 2: Analyze F_k for a specific stage k ≥ 5 -/
def analyzeFk (k : Nat)
    (N_prev : GraphInterface.FlowNetwork)
    (R_prev : List (PedigreePath (k-1)))
    (μ_prev : PedigreePath (k-1) → Rat)
    (X : SparseRecursiveMIR n)
    (hk : k ≥ 5) : IO (FkAnalysisResult k × StageResult n k) := do

  IO.println s!"=== Step 2: Analyzing F_{k} ==="

  let layerKm1 := nodes_in_layer (k - 1)
  let layerK := nodes_in_layer k

  -- Build vertex mapping for N_prev
  let vertexToNodeMap : Nat → Option Node := fun vid =>
    -- TODO: Implement proper mapping based on N_prev structure
    none
  ...
  -- Compute arc capacities C(L) for all potential arcs
  IO.println s!"Computing arc capacities for layer {k-1} → {k}..."
  let mut arcInfoList : List ArcInfo := []

  for srcNode in layerKm1 do
    for tgtNode in layerK do
      if isPermittedArcForLayer srcNode tgtNode then
        let arcInfo ← computeArcCapacityWithPath N_prev srcNode tgtNode X vertexToNodeMap nodeToVertexMap
        if arcInfo.capacity > 0 then
          arcInfoList := arcInfo :: arcInfoList

  IO.println s!"Found {arcInfoList.length} arcs with positive capacity"

  -- Count arcs with unique paths
  let uniquePathArcs := arcInfoList.filter (·.uniquePathInfo.hasUniquePath)
  IO.println s!"  {uniquePathArcs.length} arcs have unique paths"

  -- Construct F_k network
  IO.println s!"Constructing F_{k} network..."
  let F_k ← constructFkNetwork k N_prev R_prev μ_prev X arcInfoList

  -- Compute max-flow in F_k
  IO.println s!"Computing max-flow in F_{k}..."
  let flowResult ← GraphInterface.computeMaxFlow F_k
  let maxFlow := flowResult.maxFlowValue

  IO.println s!"Max-flow: {maxFlow}"
  IO.println s!"Expected: 1.0"

  -- Check feasibility
  if maxFlow ≠ 1 then
    return ({ R_k := [], μ := fun _ => 0, N_k := N_prev,
              R_prev := R_prev, μ_prev := μ_prev, Z_max := 0 },
            StageResult.failure s!"F_{k} not feasible: flow {maxFlow} ≠ 1")

  IO.println s!"✓ F_{k} is feasible"

  -- Compute frozen flows
  IO.println "Computing frozen flows..."
  let frozenResult ← GraphInterface.computeFrozenFlows F_k flowResult
  IO.println s!"Found {frozenResult.frozenArcs.size} frozen arcs"

  -- Build virtual source mapping
  let virtualSourceToRprev : Nat → Option (PedigreePath (k-1)) := fun vid =>
    if vid < R_prev.length then
      R_prev.get? vid
    else
      none

  -- Extract rigid pedigrees R_k
  let R_k := extractRigidPedigreesFromFk k frozenResult flowResult R_prev arcInfoList vertexToNodeMap virtualSourceToRprev

  IO.println s!"Rigid pedigrees in R_{k}: {R_k.length}"
  for (idx, P) in R_k.enum do
    IO.println s!"  P{idx+1}: μ = {P.flow}"

  let μ_k := fun (path : PedigreePath k) => path.flow

  -- Update R_{k-1} and μ_{k-1}
  IO.println s!"Updating R_{k-1}..."
  let (R_prev_updated, μ_prev_updated) := updateRprev R_prev μ_prev R_k
  ...
  let Z_max := 1 - totalRigid

  IO.println s!"Total rigid flow at stage {k}: {totalRigid}"
  IO.println s!"Z_max (flexible flow): {Z_max}"

  -- Update network N_k
  IO.println s!"Updating network N_{k}..."
  let N_k := updateNetworkCapacities N_prev R_k vertexToNodeMap

  return ({ R_k := R_k, μ := μ_k, N_k := N_k,
            R_prev := R_prev_updated, μ_prev := μ_prev_updated, Z_max := Z_max },
          StageResult.success)
...
def checkM3PMembership (X : SparseRecursiveMIR n) (hn : n ≥ 5) : IO (MembershipResult n) := do
  IO.println "========================================"
  IO.println "M3P MEMBERSHIP CHECK"
  IO.println "========================================"
  IO.println ""

  -- STEP 1a: Check P_MI membership
  ...
  -- Check if already fully rigid at stage 4
  if f4Result.R₄.isEmpty then
    IO.println "No rigid pedigrees found - all flow is flexible"
  ...
  -- STEP 2: Iterate through F_k for k = 5, 6, ..., n
  let mut currentN := f4Result.N₄
  let mut currentR := f4Result.R₄
  let mut currentμ := f4Result.μ

  for k in [5:n+1] do
    IO.println s!"----------------------------------------"

    let (fkResult, fkStatus) ← analyzeFk k currentN currentR currentμ X (by omega)

    match fkStatus with
    | StageResult.failure msg =>
      IO.println s!"✗ FAILED at F_{k}: {msg}"
      return MembershipResult.notInConvexHull msg
    | StageResult.success =>
      IO.println s!"✓ PASSED F_{k}"

      -- Check termination conditions
      if fkResult.Z_max = 0 then
        IO.println s!"✓ All flow is rigid at stage {k} - SUCCESS!"
        IO.println "X ∈ conv(Pedigrees_n)"
        return MembershipResult.inConvexHull

      -- Update state for next iteration
      currentN := fkResult.N_k
      currentR := fkResult.R_prev
      currentμ := fkResult.μ_prev

      IO.println ""

  -- If we reach stage n and still have flexible flow, it's a failure
  IO.println "✗ FAILED: Reached stage n with flexible flow remaining"
  return MembershipResult.notInConvexHull "Flexible flow remains at final stage"

-- ============================================================================
-- CONVEX DECOMPOSITION EXTRACTION
-- ============================================================================

/-- Extract convex decomposition from rigid pedigrees across all stages -/
structure ConvexDecomposition (n : ℕ) where
  pedigrees : List (PedigreePath n)
  weights : List Rat
  totalWeight : Rat
  deriving Repr

/-- Combine all rigid pedigrees from all stages into final decomposition -/
def extractConvexDecomposition (f4Result : F4AnalysisResult n)
    (fkResults : List (FkAnalysisResult k)) : ConvexDecomposition n :=
  -- Collect all rigid pedigrees that reach stage n
  let allPedigrees := f4Result.R₄  -- Start with F₄ results
  -- TODO: Extend with pedigrees from higher stages that reach n

  let weights := allPedigrees.map (·.flow)
  let totalWeight := weights.foldl (· + ·) 0

  { pedigrees := sorry,  -- Convert to PedigreePath n
    weights := weights,
...
/-- Display convex decomposition -/
def displayConvexDecomposition (decomp : ConvexDecomposition n) : IO Unit := do
  IO.println ""
  IO.println "========================================"
  IO.println "CONVEX DECOMPOSITION"
  IO.println "========================================"
  IO.println s!"Total pedigrees: {decomp.pedigrees.length}"
  IO.println s!"Total weight: {decomp.totalWeight}"
  IO.println ""
  for (idx, (P, w)) in (decomp.pedigrees.zip decomp.weights).enum do
    IO.println s!"Pedigree {idx+1}:"
    IO.println s!"  Weight: {w}"
    IO.println s!"  Path: {P.nodes}"
  IO.println "========================================"
...
def runM3PAnalysis (X : ParsedMIRData n) (hn : n ≥ 5) : IO Unit := do
  -- Convert ParsedMIRData to SparseRecursiveMIR
  -- TODO: Implement conversion
  let X_mir : SparseRecursiveMIR n := sorry

  -- Run membership check
  let result ← checkM3PMembership X_mir hn

  -- Display result
  displayMembershipResult result

  -- If successful, extract and display decomposition
  match result with
  | MembershipResult.inConvexHull =>
    -- TODO: Extract decomposition from analysis results
    IO.println "Convex decomposition extraction not yet implemented"
  | MembershipResult.notInConvexHull _ =>
    pure ()
...
/-- Example: Run M3P on the test problem from Test M3P Example.py -/
