...
lemma sum_binary_eq_one_implies_unique_one {α : Type*} [Fintype α] [DecidableEq α] (f : α → ℝ)
  ...
  by_contra hne
  let S := Finset.cons a (Finset.cons b ∅ (by simp)) (by simp) (by simp [hne.symm])
  have hsub : S ⊆ Finset.univ := by simp
  have le_sum : ∑ x ∈ S, f x ≤ ∑ x, f x :=
    Finset.sum_le_sum_of_subset_of_nonneg hsub (λ x _ => h0 x)
  have sum_S : ∑ x ∈ S, f x = f a + f b := by
    simp [S, ha, hb]
    rfl
  rw [sum_S] at le_sum
  linarith [hsum]
...
theorem unique_insertion_sequence {n : ℕ} (X : IntegerMIR n) (k : Fin n) (hk : k.1 ≥ 3) :
  ...
  let f : S → ℝ := λ s => X.x (s : Fin n × Fin n × Fin n)
  have hsum : ∑ s ∈ S, f s = 1 := by
    convert X.eq6 k hk
    · ext t; simp [f]
    · rw [Finset.sum_attach]
  have h0 : ∀ s ∈ S, f s ≥ 0 := λ s _ => X.nonneg (s : Fin n × Fin n × Fin n)
  have hbin : ∀ s ∈ S, f s = 0 ∨ f s = 1 := λ s _ => X.binary (s : Fin n × Fin n × Fin n)
