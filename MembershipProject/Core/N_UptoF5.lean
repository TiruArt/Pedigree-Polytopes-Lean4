...
namespace MembershipProject.Core.F5Visualization
...
def buildF4Network (x4 x5 : List (Node × Rat)) : Network :=
  ...
  let source_edges := x4.map fun (n4, _) =>
    { source := source, target := n4, capacity := (1000 : Rat) }

  -- T₄ → T₅ edges
  let t4_t5_edges := x4.flatMap fun (n4, _) =>
    x5.filterMap fun (n5, _) =>
      if validArc_4_to_5 n4.i n4.j n5.i n5.j then
        some { source := n4, target := n5, capacity := (1000 : Rat) }
      else none

  -- T₅ → sink edges
  let sink_edges := x5.map fun (n5, _) =>
    { source := n5, target := sink, capacity := (1000 : Rat) }
...
-/
def buildN4_L (n4 : Network) (src_node : Node) (_x5 : List (Node × Rat)) : Network :=
...
def buildF5Network (x5 x6 : List (Node × Rat))
  ...
  let virtual_sources := r4.mapIdx fun idx rp =>
...
def printNetwork (net : Network) (title : String) : IO Unit := do
  ...
  IO.println title
  IO.println s!"{String.ofList (List.replicate 60 '=')}"
...
def analyzeF4 (net : Network) (x4 _x5 : List (Node × Rat)) : IO Unit := do
  IO.println "\nF₄ Network Analysis:"
...
def analyzeN4_L (net : Network) (src_node tgt_node : Node) (_x5 : List (Node × Rat)) : IO Unit := do
  IO.println s!"\nN₄(L) for L = {printNode src_node} → {printNode tgt_node}:"
  IO.println s!"  Restriction: Delete all T₅ nodes except {printNode src_node}"

  -- (deleted nodes would be computed from the x5 parameter if needed)
  IO.println s!"  Restriction applied to T₅ layer"
...
def analyzeF5 (net : Network) (r4 : List RigidPedigree) : IO Unit := do
  ...
  let idxR4 := r4.mapIdx fun idx rp => (idx, rp)
  for (idx, rp) in idxR4 do
...
-- Run with: #eval testData
-- (Uncomment the line below to execute)
-- #eval testData

end MembershipProject.Core.F5Visualization
